 ...
 * @returns {{text: string, start: number, end: number}[]} Ordered chunks and
 * source offsets in the normalized input. Empty input → [].
 */
export function chunkTextWithLocations(text, {
  ...
  while (start < clean.length && chunks.length < maxChunks) {
    ...
    const rawChunk = clean.slice(start, end);
...
export function chunkText(text, opts) {
  return chunkTextWithLocations(text, opts).map((chunk) => chunk.text);
}
